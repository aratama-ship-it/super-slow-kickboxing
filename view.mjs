...
import {MOVES,gloveLocal,slipOffset,localToWorld,stanceRole,clamp} from './core.mjs';
...
export function createView(container){
  ...
  for(let index=0;index<2;index++){
    const root=new THREE.Group();scene.add(root);const a={root,arms:{},gloves:{},elbows:{},shoulders:{},legs:[],head:null,chest:null};
    ...
    if(!player){
      ...
      a.chest=mesh(new THREE.LatheGeometry(profile,32),skin,root);a.chest.position.y=.96;a.chest.scale.z=.64;
      ellipsoid(root,skin,[.14,1.3,.11],[.17,.12,.10]);ellipsoid(root,skin,[-.14,1.3,.11],[.17,.12,.10]);
      const neck=mesh(new THREE.CylinderGeometry(.075,.1,.15,20),skin,root);neck.position.set(0,1.47,0);
      ...
      const pelvis=mesh(new THREE.CylinderGeometry(.205,.26,.27,24),shorts,root);pelvis.position.y=.87;pelvis.scale.z=.72;
      const belt=mesh(new THREE.CylinderGeometry(.208,.218,.055,24),ivory,root);belt.position.y=1.005;belt.scale.z=.74;
  ...
  function render(s){
    ...
    for(let i=0;i<2;i++){
      ...
      const headX=slipOffset(f);
      if(a.head){a.head.position.x=headX;a.head.rotation.z=-headX*.3;a.head.position.z=drive*.35;a.chest.rotation.z=-headX*.12;}
      for(const leg of a.legs){
        const sign=leg.side==='L'?1:-1,lead=stanceRole(f,leg.side)==='lead',depth=lead?.15:-.15;
        const hip=[sign*.15,.83,0],knee=[sign*.19,.48,depth*.67],foot=[sign*.21,.12,depth];
        setSegment(leg.thigh,hip,knee);setSegment(leg.shin,knee,foot);leg.kneeJoint.position.set(...knee);
        leg.boot.position.set(foot[0],.075,foot[2]+.06);leg.hem.position.set(sign*.14,.78,depth*.16);
      }
      for(const side of ['L','R']){
        const sign=side==='L'?1:-1,hand=gloveLocal(f,side),shoulderDepth=stanceRole(f,side)==='lead'?.08:-.05;
        const shoulder=[sign*.3+headX*.3,1.39,drive+shoulderDepth];
