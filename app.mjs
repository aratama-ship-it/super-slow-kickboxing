...
function updateLabUI(){
  ...
  if(labRun?.impact){
    ...
    $('lab-after').textContent=definition.id==='jab-right-parry'
      ...
      : definition.id==='jab-left-parry'
        ? (remaining>0?(labRun.rebound?'相手の左拳：前へ '+Math.round(labRun.rebound.forward*100)+'cm・画面右へ '+Math.round(labRun.rebound.rightward*100)+'cm・下へ '+Math.round(labRun.rebound.downward*100)+'cm / 相手の左腕のみ使用不能 '+remaining.toFixed(1)+' TU':'左グローブへ接触 / 青い拳は前進しながら画面右へ外れ始めた'):'相手の左腕・自分の左手：復帰')
...
async function boot(){
  try{const {createView}=await import('./view.mjs?v=0.52');viewFactory=createView;view=createView($('stage'),viewOptions());view.render(state);updateUI();}
