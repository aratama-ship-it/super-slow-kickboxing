import {
  ...
  startMatch,
  tick,
} from './core.mjs?v=0.15';

export const LAB_CASES=Object.freeze([
  Object.freeze({
    id:'jab-right-parry',number:'01',title:'相手の左ジャブ × 右手パーリング',
    question:'左の前足とジャブを同時に進め、右手を真上から小さく落として進行80%付近の拳を下へ外せるか。',
    defense:'右手パーリング',defenseAt:3,
    conditions:Object.freeze(['パンチの間合い','相手は頭へ左ジャブ','ジャブの軌道が見える2.7 TU後まで待つ','開始3.0 TUで右手パーリング','拳の進行80〜82.5%で接触','他の入力なし']),
    expected:'左の前足は拳の伸びと同時に前へ出る。右手は内側へ入れず、ガード位置の真上から小さく落とす。相手の左拳は自分へ進みながら下へ外れ、ダメージ0。相手の左腕だけが4 TU使用不能。',
  }),
  Object.freeze({
...
function completeChecks(run){
  ...
  if(definition.id==='jab-right-parry')return [
    {label:'左前足が拳と同時に進み、進行80%到達後に接触',pass:run.defenseIssuedAt>MOVES.jab.cue&&run.leadFootPeak>=.075&&event?.type==='block'&&event?.defense==='parry'&&event.punchProgress>=PARRY.contactProgress.jab&&event.punchProgress<=PARRY.contactProgress.jab+PARRY.progressTolerance},
...
export function createLabRun(caseIndex=0){
  ...
  return {definition,caseIndex,state,status:'running',defenseIssued:false,defenseIssuedAt:null,parryStart:null,tapStart:null,leadFootPeak:0,impact:null,rebound:null,checks:[]};
}
...
export function advanceLabRun(run,dt=STEP){
  ...
  run.leadFootPeak=Math.max(run.leadFootPeak,leadFootMotion(run.state.fighters[1]).forward);
