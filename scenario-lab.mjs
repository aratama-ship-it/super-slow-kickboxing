...
function completeChecks(run){
  ...
  if(definition.id==='jab-right-parry')return [
    {label:'ジャブが見えてから入力し、進行80%到達後に接触',pass:run.defenseIssuedAt>MOVES.jab.cue&&event?.type==='block'&&event?.defense==='parry'&&event.punchProgress>=PARRY.contactProgress.jab&&event.punchProgress<=PARRY.contactProgress.jab+PARRY.progressTolerance},
