...
export const LAB_CASES=Object.freeze([
  Object.freeze({
    ...
    expected:'左ジャブの拳中心は額より10cm低い顔中央へ向かう。左右の拳は構え中から小さく動き続け、ジャブ固有の引き動作を見せない。2.7 TUから左拳と左前足、腰、胸、頭、肩線が同時に小さく前へ出て、後ろ足は残る。右手は内側へ入れず、ガード位置の真上から小さく落とす。相手の左拳は自分へ進みながら下へ外れ、ダメージ0。相手の左腕だけが4 TU使用不能。',
  }),
  Object.freeze({
